...
pub async fn scrape() -> Result<Vec<Anime>> {
    Scraper::default().scrape().await
}

struct Scraper {
    client: Client,
    noredirect_client: Client,
}

impl Default for Scraper {
    fn default() -> Self {
        Self {
            client: build_http_client(true).unwrap(),
            noredirect_client: build_http_client(false).unwrap(),
        }
    }
}
...
impl Scraper {
    async fn scrape(&self) -> Result<Vec<Anime>> {
        let all_anime_list_rsp = self
            .client
        ...
        let mut all_anime = Vec::with_capacity(256);
        for element in document.select(&selector) {
            let url = format!("https://shinden.pl{}", element.attr("href").unwrap());
            if let Ok(anime) = self.scrape_anime_details(&url).await {
                println!("{anime}\n=====");
    ...
    async fn scrape_anime_details(&self, url: &str) -> Result<Anime> {
        let anime_page_rsp = self
            .client
            .get(url)
            .send()
            .await?
        ...
        let ogladajanime_url = self.make_ogladajanime_url(&title).await;
    ...
    async fn make_ogladajanime_url(&self, title: &str) -> String {
        ...
        if let Ok(res) = self.validate_ogladajanime_url(&url).await {
            if res {
    ...
    async fn validate_ogladajanime_url(&self, url: &str) -> Result<bool> {
        let rsp = self.noredirect_client.get(url).send().await?;
        match rsp.status() {
...
mod tests {
    ...
    async fn scrape_anime() {
        ...
        let anime = scraper.scrape_anime_details(url).await.unwrap();
    ...
    #[tokio::test]
    async fn make_ogladajanime_url_positive() {
        let scraper = Scraper::default();

        assert_eq!(
            scraper.make_ogladajanime_url("Fate/strange Fake").await,
            "https://ogladajanime.pl/anime/fate-strange-fake"
        );
        assert_eq!(
            scraper
                .make_ogladajanime_url("[Oshi no Ko] 3rd Season")
                .await,
            "https://ogladajanime.pl/anime/oshi-no-ko-3rd-season"
        );
        assert_eq!(
            scraper
                .make_ogladajanime_url("Cardfight!! Vanguard: Divinez Genma Seisen-hen")
                .await,
    ...
    #[tokio::test]
    async fn make_ogladajanime_url_negative() {
        let scraper = Scraper::default();
        assert_eq!(
            scraper
                .make_ogladajanime_url("jujutsu-kaisen-3rd-season")
                .await,
            ""
        );
    }
